...
def RidgeRegression(X, y, lam=1, preprocessing = False):
    """
    ...
    # Preprocessing (It seems sklearn implementation only centers the data)
    if preprocessing:
        y = y - np.mean(y)
...
if __name__ == '__main__':
    ...
    with open("wRR_[lambda].csv", "w") as text_file:
        for w in wRR:
            text_file.write(("%.1f\n") % w)

    with open("active_[lambda]_[sigma2].csv", "w") as text_file:
